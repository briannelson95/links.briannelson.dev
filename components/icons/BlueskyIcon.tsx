import React from 'react'

export default function BlueskyIcon() {
    return (
        <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" x="0px" y="0px"viewBox="0 0 512 512" className='size-6' xmlSpace="preserve">
            <style type="text/css">
            {`.st0{fill:#000000;}`}
            </style>
            <g>
            <path className="st0" d="M111,60.9c58.7,44.1,121.8,133.4,145,181.4c23.2-47.9,86.3-137.3,145-181.4c42.4-31.8,111-56.4,111,21.9
                c0,15.6-9,131.3-14.2,150.1c-18.3,65.3-84.9,82-144.1,71.9c103.5,17.6,129.9,76,73,134.4c-108,110.9-155.3-27.8-167.4-63.4
                c-2.2-6.5-3.3-9.6-3.3-7c0-2.6-1.1,0.5-3.3,7c-12.1,35.5-59.4,174.2-167.4,63.4c-56.9-58.4-30.5-116.8,73-134.4
                C99.1,314.9,32.5,298.3,14.2,233C9,214.2,0,98.5,0,82.8C0,4.5,68.6,29.1,111,60.9L111,60.9z"/>
            </g>
        </svg>
    )
}
